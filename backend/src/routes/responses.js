...
router.post('/generate', asyncHandler(async (req, res, next) => {
  ...
  try {
    // Support both experimentId (camelCase) and experiment_id (snake_case) for compatibility
    const experimentId = req.body.experimentId || req.body.experiment_id;
    const prompt = req.body.prompt;
    const parameters = req.body.parameters || req.body;
    const count = req.body.count || req.body.response_count || 1;
    ...
    // If no experiment ID provided, try to create a temporary one
    if (!experimentId && prompt) {
      logger.info('No experiment ID provided, generating single response for prompt');
      ...
      // Generate single response without experiment
      const result = await responseController.generateSingleResponse({
        prompt,
        ...responseParams
      });
